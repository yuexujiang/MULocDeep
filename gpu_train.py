...
def endpad(seqfile, labelfile, pssmdir=""):
        new_pssms = []
        labels = []
        mask_seq = []
        ids=[]
...
def train_MULocDeep(lv1_dir,lv2_dir,pssm_dir,output_dir,foldnum):
    ...
    [train_x, train_y, train_mask, train_ids] = endpad(
        lv2_dir+"uniseq_nr40_10clas_lv2only_v2_train_fold" + str(foldnum) + "_seq",
        lv2_dir+"uniseq_nr40_10clas_lv2only_v2_train_fold" + str(foldnum) + "_label",
        pssm_dir)
    [val_x, val_y, val_mask,val_ids] = endpad(
        lv2_dir+"uniseq_nr40_10clas_lv2only_v2_val_fold" + str(foldnum) + "_seq",
        lv2_dir+"uniseq_nr40_10clas_lv2only_v2_val_fold" + str(foldnum) + "_label",
        pssm_dir)

    # get big data
    [train_x_big, train_y_big, train_mask_big,train_ids_big] = endpad(
        lv1_dir+"deeploc_40nr_train_fold" + str(foldnum) + "_seq",
        lv1_dir+"deeploc_40nr_train_fold" + str(foldnum) + "_label",
        pssm_dir)
    [val_x_big, val_y_big, val_mask_big,val_ids_big] = endpad(
        lv1_dir+"deeploc_40nr_val_fold" + str(foldnum) + "_seq",
        lv1_dir+"deeploc_40nr_val_fold" + str(foldnum) + "_label",
        pssm_dir)
    ...
    for i in range(50):
        ...
        fitHistory_batch_big = model_big.fit([train_x_big, train_mask_big.reshape(-1, 1000, 1)],
                                             ...
                                             callbacks=[checkpoint_acc_big_lev1], verbose=1)
