...
import argparse
import sys
# sys.path.append('./review_2ndRound_5/')                     #modify
...
def endpad(seqfile, pssmdir):
    ...
    index = 0
    while line != '':
        pssmfile = pssmdir + line[1:].strip() + "_pssm.txt"  # modify
...
map_lv2 = {1.0: "Cytoplasmic vesicle", 1.1: "Cytoplasm, cytoskeleton", 1.2: "Cytoplasm, myofibril",
           1.3: "Cytoplasm, cytosol", 1.4: "Cytoplasm, perinuclear region",
           1.5: "Cytoplasm, cell cortex", 1.6: "Cytoplasmic granule", 1.7: "Cytoplasm, P-body",
           0.0: "Nucleus, nucleolus", 0.1: "Nucleus, nucleoplasm", 0.2: "Nucleus membrane", 0.3: "Nucleus matrix",
           0.4: "Nucleus speckle", 0.5: "Nucleus, PML body",
           0.6: "Nucleus, Cajal body", 0.7: "Chromosome",
           3.0: "Mitochondrion inner membrane", 3.1: "Mitochondrion intermembrane space",
           3.2: "Mitochondrion outer membrane", 3.3: "Mitochondrion matrix",
           3.4: "Mitochondrion membrane",
           5.0: "Endoplasmic reticulum lumen", 5.1: "Endoplasmic reticulum membrane",
           5.2: "Endoplasmic reticulum-Golgi intermediate compartment",
           5.3: "Microsome", 5.4: "Sarcoplasmic reticulum",
           2.0: "Secreted, exosome", 2.1: "Secreted, extracellular space",
           7.0: "Golgi apparatus, trans-Golgi network", 7.1: "Golgi apparatus, cis-Golgi network",
           7.2: "Golgi apparatus membrane", 7.3: "Golgi apparatus, Golgi stack membrane",
           4.0: "Membrane, clathrin-coated pit", 4.1: "Membrane, coated pit", 4.2: "Membrane raft",
           4.3: "Membrane, caveola", 4.4: "Cell membrane", 4.5: "Cell surface",
           8.0: "Lysosome membrane",
           9.0: "Peroxisome membrane",
           6.0: "Plastid, amyloplast", 6.1: "Plastid, chloroplast membrane", 6.2: "Plastid, chloroplast stroma",
           6.3: "Plastid, chloroplast thylakoid lumen",
           6.4: "Plastid, chloroplast thylakoid membrane"
           }

name = ["Nucleus", "Cytoplasm", "Secreted", "Mitochondrion", "Membrane", "Endoplasmic", "Plastid", "Golgi_apparatus",
        "Lysosome", "Peroxisome"]
...
def main():
    ...
    for foldnum in range(8):
        ...
        model_small.load_weights(
            './review_2ndRound_6/models/cpu_model/fold' + str(foldnum) + '_big_lv1_acc-weights.hdf5')
        # model_small.load_weights(
        #     './review_2ndRound_5/models/gpu_model/fold' + str(foldnum) + '_small_lv2_acc-weights.hdf5')
    ...
    #                   [0.5, 0.1, 0.5, 0.5, 0.5, 0.5, 1, 1],
    #                   [0.5, 0.5, 0.5, 0.5, 0.5, 1, 1, 1],
    #                   [0.5, 0.5, 0.5, 0.1, 0.5, 1, 1, 1],
    #                   [0.1, 0.03, 0.5, 0.07, 1, 1, 1, 1],
    #                   [0.03, 1, 1, 1, 1, 1, 1, 1],
    #                   [0.03, 1, 1, 1, 1, 1, 1, 1]])
    # #custom2
    # cutoff = np.array([[0.5, 0.5, 0.5, 0.3, 0.5, 0.4, 0.3, 0.5],
    #                   [0.5, 0.5, 0.5, 0.5, 0.4, 0.5, 0.5, 0.3],
    #                   [1, 0.5, 1, 1, 1, 1, 1, 1],
    #                   [0.5, 0.5, 0.5, 0.5, 0.5, 1, 1, 1],
    #                   [0.5, 0.1, 0.4, 0.4, 0.4, 0.3, 1, 1],
    #                   [0.4, 0.5, 0.2, 0.5, 0.5, 1, 1, 1],
    #                   [0.5, 0.5, 0.5, 0.1, 0.5, 1, 1, 1],
    #                   [0.1, 0.03, 0.5, 0.07, 1, 1, 1, 1],
    #                   [0.03, 1, 1, 1, 1, 1, 1, 1],
    #                   [0.03, 1, 1, 1, 1, 1, 1, 1]])
    ...
    cutoff = np.array([[0.5, 0.5, 0.5, 0.3, 0.5, 0.4, 0.3, 0.5],
                      ...
                      [0.4, 1, 1, 1, 1, 1, 1, 1]])
    # new model cross
    # cutoff = np.array([[0.9, 0.5, 0.9, 0.2, 0.8, 0.4, 0.3, 0.9],
    #                   [0.5, 0.9, 0.7, 0.7, 0.4, 0.5, 0.7, 0.3],
    #                   [0.88, 0.9, 0., 0., 0., 0., 0., 0.],
    #                   [0.9, 0.5, 0.8, 0.9, 0.6, 0., 0., 0.],
    #                   [0.5, 0.1, 0.4, 0.4, 0.4, 0.3, 0., 0.],
    #                   [0.4, 0.3, 0.2, 0.2, 0.2, 0., 0., 0.],
    #                   [0.2, 0.6, 0.9, 0.1, 0.5, 0., 0., 0.],
    #                   [0.1, 0.1, 0.5, 0.3, 0., 0., 0., 0.],
    #                   [0.4, 0., 0., 0., 0., 0., 0., 0.],
    #                   [0.4, 0., 0., 0., 0., 0., 0., 0.]])

    #web cutoff
    # cutoff = np.array([[0.9, 0.1, 0.9, 0.3, 0.4, 0.1, 0.1, 0.9],
    #                    [0.3, 0.6, 0.2, 0.5, 0.1, 0.1, 0.1, 0.2],
    #                    [1, 0.9, 1, 1, 1., 1., 1., 1.],
    #                    [0.9, 0.6, 0.8, 0.7, 0.9, 1., 1., 1.],
    #                    [0.2, 0.1, 0.1, 0.3, 0.1, 0.1, 1., 1.],
    #                    [0.3, 0.9, 0.2, 0.5, 0.4, 1., 1., 1.],
    #                    [0.4, 0.8, 0.9, 0.6, 0.9, 1., 1., 1.],
    #                    [0.1, 0.1, 0.6, 0.1, 1., 1., 1., 1.],
    #                    [0.3, 1., 1., 1., 1., 1., 1., 1.],
    #                    [0.6, 1., 1., 1., 1., 1., 1., 1.]])
